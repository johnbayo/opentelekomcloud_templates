...
resource "null_resource" "certificate" {
  ...
  provisioner "local-exec" {
    command = <<EOT
       ...
       curl -si -d @json_file -H "Content-Type: application/json" -H "X-Auth-Token:$TOKEN" https://${var.resource_type}.eu-de.otc.t-systems.com/${var.api_version}/${data.opentelekomcloud_identity_project_v3.project_id.id}/${var.resource_url} > .${var.domain[count.index]}-id.txt;
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
       ...
       curl -ki -X DELETE -H "X-Auth-Token:$TOKEN" https://${var.resource_type}.eu-de.otc.t-systems.com/${var.api_version}/${data.opentelekomcloud_identity_project_v3.project_id.id}/${var.resource_url}/$id;
